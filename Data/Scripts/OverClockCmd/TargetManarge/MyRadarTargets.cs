...
namespace SuperBlocks.Controller
{
    ...
    public sealed class MyRadarTargets : UpdateableClass
    {
        ...
        {
            if (Utils.NullEntity(DetectorProcess)) return;
            Range = distance * 1.5;
        }
        public Target 得的最近向我靠近最快的目标(IMyTerminalBlock RequstBlock)
        {
            if (Utils.IsNull(RequstBlock) || Utils.IsNullCollection(TargetsList) || Range < 10) return null;
        ...
        public Target 得的最近向我靠近最快的目标(IMyTerminalBlock RequstBlock, Func<IMyEntity, bool> Filter = null, Func<IMyEntity, double> FilterPriority = null)
        {
            if (RequstBlock == null || Utils.IsNullCollection(TargetsList) || Range < 10) return null;
            var ent_tgs = TargetsList.Where(tg => Filter?.Invoke(tg.Entity) ?? true)?.ToList();
            if (Utils.IsNullCollection(ent_tgs)) return null;
        ...
        #endregion
        #region 外部可调用属性
        public Target 当前目标 { get; private set; }
        #endregion
        #region 更新函数       
        protected override void UpdateFunctions(IMyTerminalBlock CtrlBlock)
        {
            ...
            if (updatecounts % 11 == 0)
            {
                ...
                if (Utils.IsNullCollection(entities)) return;
                DetectedEntities.UnionWith(entities);
                List<IMyEntity> 敌人 = entities.Where(t => 可能敌对的目标(t, CtrlBlock))?.ToList();
                if (Utils.IsNullCollection(敌人)) { 当前目标 = null; return; }
                Enemies.RemoveWhere(Utils.NullEntity);
                Enemies.UnionWith(敌人);
                if (Utils.IsNullCollection(Enemies)) { 当前目标 = null; return; }
                TargetsList.UnionWith(Enemies.ToList().ConvertAll(t => new Target(t, CtrlBlock)));
        ...
        {
            if (Utils.NullEntity(target)) return false;
        ...
        private bool 目标敌对(IMyEntity Ent, IMyTerminalBlock DetectorProcess)
        {
            ...
            if (Ent is IMyMeteor || Utils.是否是导弹(Ent)) return 目标朝我靠近(Ent, DetectorProcess);
        ...
        {
            var blocks = Utils.GetTs<IMyFunctionalBlock>(MyAPIGateway.TerminalActionsHelper.GetTerminalSystemForGrid(Grid), 供能方块);
        ...
        private bool TargetInRange(Target target, IMyTerminalBlock DetectorProcess)
        {
        ...
        private HashSet<Target> TargetsList { get; } = new HashSet<Target>();
