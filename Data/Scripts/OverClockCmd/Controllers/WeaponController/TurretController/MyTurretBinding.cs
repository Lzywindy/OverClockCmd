...
namespace SuperBlocks.Controller
{
    public sealed class MyTurretBinding
    {
        ...
        {
            ReferWeapon();
            TargetPredict.CalculateDirection(Me, CurrentWeapons, ref Parameters);
        ...
        public void RunningAutoFire()
        {
            ...
            else count = Config.firegap;
        }
        public void SetFire(bool FireWeapons)
        {
            if (!FireWeapons) { WeaponsToFire.Clear(); foreach (var weapon in CurrentWeapons) MyWeaponAndTurretApi.FireWeapon(weapon, false); return; }
            if (Config == null) { foreach (var Weapon in Weapons) { WeaponsToFire.Clear(); MyWeaponAndTurretApi.FireWeapon(Weapon, false); } return; }
            ...
            else { if (FireWeapons == true && Utils.Common.IsNullCollection(WeaponsToFire) && WeaponsToFire.All(MyWeaponAndTurretApi.CanFire)) { foreach (var Weapon in CurrentWeapons) { if (Utils.Common.NullEntity(Weapon)) continue; MyWeaponAndTurretApi.FireWeapon(Weapon, false); WeaponsToFire.Enqueue(Weapon); } count = 0; } }
        }
        private int count = 0;
        ...
        string WeaponName = "DefaultWeapon";
        string AmmoName = "DefaultAmmo";
        MyWeaponParametersConfig Parameters = default(MyWeaponParametersConfig);
        #endregion
    }
